...
// Eq.(8)
inline double gauss(const double x, const double mean, const double sigma)
{
    return (exp((-(x - mean) * (x - mean)) / (2 * sigma * sigma)) / (sqrt(constant::PI * 2.0) * sigma));
}

void MakeGaussianVector(const double sigma, std::vector<double> &GAU)
{
    ...
    for (int y = 0; y < img_h; ++y) {
        for (int x = 0; x < img_w; ++x) {
            ...
            for (auto [step, pos] = std::tuple{0, cv::Point2f(x, y)}; step < kernel_half; ++step) {
                ...
                const int py          = static_cast<int>(round(pos.y));
                cv::Vec3f tmp         = -etf.flowField.at<cv::Vec3f>(py, px);
                const auto direction = cv::Point2f(tmp[1], tmp[0]);
...
#pragma omp parallel for
    for (int y = 0; y < dst.rows; ++y) {
        for (int x = 0; x < dst.cols; ++x) {
            ...
            const double v_c          = gau_c_acc / gau_c_weight_acc;
            const double v_s          = gau_s_acc / gau_s_weight_acc;
...
#pragma omp parallel for
    for (int y = 0; y < dst.rows; ++y) {
        for (int x = 0; x < dst.cols; ++x) {
            const bool condition = src.at<float>(y, x) < tau;
            dst.at<uchar>(y, x) = binary2color.at(condition);
        }
    }
}
