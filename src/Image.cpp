...
bool Image::read(const char* filename, const int channel_force) {
	uint8_t* stbData = stbi_load(filename, &w, &h, &channels, channel_force);
	if (stbData == nullptr) {
		data = nullptr;
		return false;
	}
	...
	// Copy into a new[]-allocated buffer so that ownership of `data` is
	// uniform across the whole class (ctor(w,h,channels), copy ctor,
	// operator=, resizeNN, crop all use new[]/delete[]). The stb buffer
	// is freed right away with its own matching deallocator.
	data = new uint8_t[size];
	memcpy(data, stbData, size);
...
bool Image::write(const char* filename) const {
	...
	if (success != 0) {
		...
		Logger::log("\033[32mWrote \033[36m", filename, "\033[0m, ", w, ", ", h, ", ", channels, ", ", size);
		return true;
	}
	// Bold red: "Failed to write", Cyan: filename, then image dimensions
	Logger::err("\033[31;1mFailed to write \033[36m", filename, "\033[0m, ", w, ", ", h, ", ", channels, ", ", size);
	return false;
}
...
Image& Image::apply_proportion_transformation_region_fraction(
    ...
    const auto thresholdIndex = static_cast<size_t>((regionPixelCount - 1) * proportion);
...
#pragma omp parallel for
    for (size_t i = 0; i < size; i += channels) {
        for (int c = 0; c < 3; ++c) {
            auto val = static_cast<float>(data[i + c]); // Explicit cast to float
...
Image& Image::resizeNN(const uint16_t nw, const uint16_t nh) {
	...
	for(uint16_t y = 0; y < nh; ++y) {
		// Clamp to [0, h-1]: std::round pushes sy to h (one past the last
		// valid row) whenever scaleY >= 2, i.e. whenever we enlarge 2x or more.
		const auto sy = std::min<uint16_t>(
			static_cast<uint16_t>(std::round(static_cast<float>(y) / scaleY)), h - 1);
		for(uint16_t x = 0; x < nw; ++x) {
			// Same clamp on the x axis, for the same reason.
			const auto sx = std::min<uint16_t>(
				static_cast<uint16_t>(std::round(static_cast<float>(x) / scaleX)), w - 1);
